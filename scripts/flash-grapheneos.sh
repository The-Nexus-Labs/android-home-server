...
[[ -n "$release_dir" ]] || die "official GrapheneOS flash script not found; run ./scripts/download-assets.sh --download first"
...
if ! fastboot getvar unlocked 2>&1 | tr '[:upper:]' '[:lower:]' | grep -qE 'unlocked:[[:space:]]*(yes|true)'; then
  print_manual_block "Bootloader is still locked, so flashing is blocked.

What to do:
  1. Boot into Android.
  2. Enable OEM unlocking in Developer options.
  3. Run the unlock step and confirm the on-device prompt.
