...
step_state_key() {
  printf 'STATE_ROOT_READY\n'
}

step_is_done() {
  adb_root id >/dev/null 2>&1
}

step_guide() {
  cat <<'EOF'
This step installs the Magisk app, patches the current init_boot image, and flashes the patched image.

After the phone reboots:
  1. Unlock the phone.
  2. Open Magisk.
  3. If Magisk asks for additional setup, approve it and let the phone finish.
  4. Re-enable USB debugging if Android asks again.
  5. Accept the ADB authorization prompt.
EOF
}

step_apply() {
  local magisk_apk_name host_patch_name magisk_apk host_patch grapheneos_init_boot_img init_boot_img patched_img busybox_tmp
...
Then continue with the remaining provisioning steps or rerun make interactive.
EOF
}
