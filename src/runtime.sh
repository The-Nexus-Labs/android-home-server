...
trigger_termux_root_request() {
  ...
  request_script=/data/data/com.termux/files/home/request-root.sh

  adb_root "cat > '$request_script' <<'EOF'
#!/data/data/com.termux/files/usr/bin/bash
su -c true >/dev/null 2>&1 || exit 1
EOF
chown ${termux_uid}:${termux_uid} '$request_script' && chmod 700 '$request_script'"
}
